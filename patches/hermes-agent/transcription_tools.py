...
DEFAULT_OPENROUTER_STT_MODEL = os.getenv("STT_OPENROUTER_MODEL", "google/gemini-3.1-flash-lite")
...
def _has_openrouter_audio_backend() -> bool:
    """Return True when OpenRouter STT has usable credentials."""
    try:
        _resolve_openrouter_audio_client_config()
        return True
    except ValueError:
        return False
...
def _get_provider(stt_config: dict) -> str:
    ...
    if explicit:
        ...
        if provider == "openrouter":
            if _HAS_OPENAI and _has_openrouter_audio_backend():
                return "openrouter"
            logger.warning(
...
def transcribe_audio(file_path: str, model: Optional[str] = None) -> Dict[str, Any]:
    ...
    return {
        ...
        "error": (
            ...
            "set ELEVENLABS_API_KEY for ElevenLabs Scribe, set OPENROUTER_API_KEY for "
            "OpenRouter multimodal audio, or set VOICE_TOOLS_OPENAI_KEY or OPENAI_API_KEY "
...
def _resolve_openrouter_audio_client_config() -> tuple[str, str]:
    """Return OpenRouter STT config using explicit config or env credentials."""
    stt_config = _load_stt_config()
    openrouter_cfg = stt_config.get("openrouter", {})
    cfg_api_key = str(openrouter_cfg.get("api_key", "") or "").strip()
    cfg_base_url = str(openrouter_cfg.get("base_url", "") or "").strip()
    if cfg_api_key:
        return cfg_api_key, (cfg_base_url or OPENROUTER_STT_BASE_URL)

    env_base_url = cfg_base_url or OPENROUTER_STT_BASE_URL
    direct_api_key = (get_env_value("OPENROUTER_API_KEY") or get_env_value("OPENAI_API_KEY") or "")
    if direct_api_key:
        return str(direct_api_key).strip(), env_base_url

    raise ValueError("Neither stt.openrouter.api_key in config nor OPENROUTER_API_KEY/OPENAI_API_KEY is set")


def _openrouter_audio_instruction(language: Optional[str] = None) -> str:
    base = (
        "Transcreva este ?udio com m?xima fidelidade. "
        "Retorne apenas o texto transcrito, sem resumo, sem explica??es, "
        "sem markdown e sem metadados."
    )
    if language:
        return f"{base} O idioma esperado ? {language}."
    return base


def _prepare_openrouter_audio(file_path: str) -> tuple[Optional[str], Optional[str], Optional[str], Optional[str]]:
    """Return (path_to_send, format, temp_dir, error) for OpenRouter chat-audio STT."""
    ...
    if suffix in format_map:
        return file_path, format_map[suffix], None, None

    work_dir = tempfile.mkdtemp(prefix="hermes_openrouter_stt_")
    converted_path, error = _prepare_local_audio(file_path, work_dir)
    if error:
        shutil.rmtree(work_dir, ignore_errors=True)
        return None, None, None, error
    return converted_path, "wav", work_dir, None


def _extract_chat_completion_text(completion: Any) -> str:
    """Normalize chat completion responses to plain transcript text."""
    ...
    if isinstance(completion, dict):
        ...
        if choices:
            ...
            text = _flatten(message.get("content")).strip()
            if text:
                return text
...
def _transcribe_openrouter(file_path: str, model_name: str) -> Dict[str, Any]:
    """Transcribe audio via OpenRouter chat-completions audio-capable models."""
    try:
        api_key, base_url = _resolve_openrouter_audio_client_config()
    except ValueError as exc:
    ...
    try:
        audio_path, audio_format, temp_dir, prep_error = _prepare_openrouter_audio(file_path)
        if prep_error:
            return {"success": False, "transcript": "", "error": prep_error}
        ...
        try:
                ...
                messages=[
                    {
                        "role": "system",
                        "content": (
                            "Voc? ? um mecanismo de transcri??o literal de fala para texto. "
                            "Nunca resuma, nunca explique e nunca invente."
                        ),
                    },
                    {
                        "role": "user",
                        "content": [
                            {"type": "text", "text": _openrouter_audio_instruction()},
                            {
                                "type": "input_audio",
                                "input_audio": {
                                    "data": audio_b64,
            ...
            transcript_text = _extract_chat_completion_text(completion)
            logger.info(
                "Transcribed %s via OpenRouter (%s, %d chars)",
                Path(file_path).name,
                model_name,
                len(transcript_text),
            )
